...
// Replace a tab's contents with header + rows, and lock it against editing.
...
function refreshSheetCopy() {
  ...
  Logger.log('Sheet copy refreshed: %s items, %s BOM lines, %s log rows.',
    snapshot.items.length, snapshot.bom_lines.length, snapshot.checkout_log.length);
}
